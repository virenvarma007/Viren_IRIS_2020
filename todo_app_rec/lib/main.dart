...
class _TodoState extends State<Todo> {
  ...
  Future<void> confirmer(String title4, DateTime date4) async {
    ...
    await flutterLocalNotificationsPlugin.show(
        0, 'Todo Item Confirmer', 'You will be notified about $title4 on $date4', notificationDetails);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
              ...
              if (m==0){
                ...
                for(int i=1; i<noteBox.length; i++){
                  BlocProvider.of<NoteBloc>(context).add(
                    NoteEvent.add(
                      Note1(noteBox.getAt(i).title,noteBox.getAt(i).description,noteBox.getAt(i).date),
                    ),
                  );
                  _showNotifications(noteBox.getAt(i).title,noteBox.getAt(i).description,noteBox.getAt(i).date);
                }
                m++;
                print('App Reloaded');
              }
        ...
        bottom: PreferredSize(child: Text("$_currentDate $_instruction ", style: TextStyle(color: Colors.white.withOpacity(1.0)))),
    ...
    return Container(
        ...
        builder: (context, noteList) {
          return ValueListenableBuilder(
            ...
            {
              String _listDate = new DateFormat.yMMMd().format(noteList[index].date);
              ...
              return Card(
                ...
                child: ListTile(
                              ...
                              if(T=='') {
                                showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                          title:Text("Item's Date is $_listDate"),
                                          content:SingleChildScrollView(child: Column(children: <Widget>[
                                            Text('Press it if you wish to see all the items for the date $_listDate?'),
                                            RaisedButton(
                                                child: Text("Select Date"),
                                                onPressed: () {
                                                  setState(() {
                                                    _dateTime =noteList[index].date;
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () {
                        if(T!='') {
                          BlocProvider.of<NoteBloc>(context).add(NoteEvent.delete(index),);
                      ...
                      setState(() {
                        if(T!='') {
                          if(index==null){
                            ...
                            Desccontroller.text= noteList[index].description;
                            _task='Update';
                          }
                          addItemDialog();
                          BlocProvider.of<NoteBloc>(context).add(NoteEvent.delete(index),);
                  ...
                  children: <Widget>[
                    ...
                    RaisedButton(
                      ...
                      onPressed: () {
                        ...
                        BlocProvider.of<NoteBloc>(context).add(
                          NoteEvent.add(
                            Note1(Titlecontroller.text,Desccontroller.text,_dateTime),
                          ),
                        );
                        if(index==null){
                        addItem(Note(Titlecontroller.text,Desccontroller.text,_dateTime));
                        _showNotifications(Titlecontroller.text,Desccontroller.text,_dateTime);
                        ...
                        Scaffold.of(context).showSnackBar(SnackBar(content: Text("$_task Successful")));
